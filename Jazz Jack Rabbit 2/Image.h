...
public:
	...
	void Render(HDC hdc);
	void Render(HDC hdc, float destX, float destY);	// 이미지 데이터를 화면에 복사
	void Render(HDC hdc, float destX, float destY, int frameX, int frameY, float scale = 1.0f);	// 이미지 데이터를 화면에 복사
